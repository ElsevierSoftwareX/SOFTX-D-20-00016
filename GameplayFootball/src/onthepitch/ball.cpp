  ...
  for (unsigned int predictTime_ms = int(timeStep * 1000.0f); predictTime_ms < ballPredictionSize_ms; predictTime_ms += int(timeStep * 1000.0f)) {
    ...
    if (predictTime_ms <= 10 && netting_enabled) {
      ...
      if (( ballIsInGoal && !betweenGoalWidth && behindBackline)) {
        ...
        std::string str_to_write = "frame: " + frame + "\ttime:" + int_to_str(match->GetActualTime_ms()) + "\t event: ";

        match->logger.writeToFile("experimental.txt", "Side netting");
...
void Ball::Process() {
  ...
  //GetDebugOverlay()->DrawRectangle(x - 5, y - 5, 10, 10, color, 255);
...
void Ball::Log2DPositionTimestamp() {
  int x, y;
  GetDebugOverlayCoord(match, positionBuffer, x, y);

  std::string frame = int_to_str(match->GetActualTime_ms() / 10);
  
  string lineStr = std::to_string(match->getCurrentRelativeTimestampMillis()) + " " + frame + " 128 " + int_to_str(x) + " " + int_to_str(y);
