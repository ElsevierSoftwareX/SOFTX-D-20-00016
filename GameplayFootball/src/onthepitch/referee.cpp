    ...
    if (fabs(ballPos.coords[0]) > pitchHalfW + lineHalfW + 0.11) {
      ...
      if (isFoul == false) {
        ...
        if (match->IsGoalScored()) {
          ...
          match->AnnotateBallOut("rimessa dal fondo"); 
          ballOut = true;
        }
        ...
        if (ballOut) {
          ...
          if (match->startedSavedShot) {
            // La palla è finita fuori e un shotSaved era partito in precedenza, dunque lo annoto
          ...
          } else if (match->startedShot) {
            // Era partito uno shot e non è stato deviato dal portiere
            match->AnnotateShotOut(match->startedShotFrame, frame, match->startedShotShooter);
            match->startedShot = false;
            match->startedSavedShot = false;
          }
        }

        // La palla è finita fuori o c'è stato goal, pertanto un eventuale passaggio iniziato non può finire
        match->startedPass = false;
        match->startedPassInCrossPosition = false;
        match->startedShotInCrossPosition = false;
    ...
    if (afterSetPieceRelaxTime_ms == 0) {
      if (fabs(ballPos.coords[1]) > pitchHalfH + lineHalfW + 0.11) {
        ...
        if (!CheckFoul()) {
          ...
          match->AnnotateBallOut("sideline");
        }
      }
    }
...
bool Referee::CheckFoul() {
  ...
  if (foul.foulType != 0 && foul.advantage == false && !foul.hasBeenProcessed) {
    ...
    if (!penalty) {
      ...
      match->AnnotateFoul(foul.foulPlayer);
      ...
      match->AnnotatePenalty(foul.foulPlayer);
    ...
    if (foul.foulType == 2) {
      ...
      match->logger.writeToFile("experimental.txt", "foul type: yellow card \tto player: " + int_to_str(foul.foulPlayer->GetID()));
    ...
    if (foul.foulType == 3) {
      ...
      match->logger.writeToFile("experimental.txt", "foul type: red card \tto player: " + int_to_str(foul.foulPlayer->GetID()));
