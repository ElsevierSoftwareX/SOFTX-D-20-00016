  ...
  if (currentAnim->touchFrame == currentAnim->frameNum) {
    ...
    if (fullBallDistance < touchableDistance && fabs(desiredBallHeight - match->GetBall()->Predict(0).coords[2]) < 1.0f) {
      ...
      if (currentAnim->functionType == e_FunctionType_Trap || (currentAnim->functionType == e_FunctionType_BallControl && CastPlayer()->HasPossession() == false)) {
        ...
        //Utile per loggare il tocco col piede o lo stop?
      }
        ...
        int frame = match->GetActualTime_ms() / 10;
        Player* player = this->CastPlayer();
        Vector3 pos = player->GetPosition();
        float x = pos.coords[0] + pitchHalfW;
        float y = pos.coords[1] + pitchHalfH;
                
        if (match->startedSavedShot) {
          // Se c'è un SavedShot in pending, lo posso annotare
          match->AnnotateSavedShot(match->startedShotFrame, match->savedShotEndFrame, match->startedShotShooter, match->savedShotPosition, match->savedShotGoalkeeper);   
        } else if (match->startedPass && match->startedPassPlayer->GetTeamID() == player->GetTeamID()) {     
          // Controllo se prima di iniziare questo tiro c'era un passaggio non segnato ancora  
          match->AnnotatePass(match->startedPassFrame, frame - 1, match->startedPassPlayer, player);
          match->AnnotateBallPossession(frame - 1, player);
          
          match->startedPass = false;
          // TODO: controllare anche i cross e i filteringPass
        }   

        match->AnnotateKickingTheBall(player);

        match->completedPass = false;       
        match->startedShot = false;
        match->startedSavedShot = false;
        match->startedPass = true;
        match->startedPassFrame = frame;
        match->startedPassPlayer = player;        

        // Per valutare se il passaggio parte in posizione buona per considerarlo cross        
        if (player->GetTeam()->GetSide() == -1) {
          // Se la squadra di player ha side == 1 allora la metà campo è quella destra
          if (x > pitchHalfW && (y < match->initCrossBottomLine || y > match->initCrossTopLine)) {
            // Se sono oltre la metà campo e sono sotto la linea della fascia oppure sopra la linea dell'altra fascia
            match->startedPassInCrossPosition = true;
            match->crossSide = true;
          }
        } else {
          // Altrimenti la metà campo è quella sinistra
          if (x < pitchHalfW && (y < match->initCrossBottomLine || y > match->initCrossTopLine)) {
            // Se sono prima della metà campo e sono sotto la linea della fascia oppure sopra la linea dell'altra fascia
            match->startedPassInCrossPosition = true;
            match->crossSide = false;
          }
        }

        if (match->startedPassInCrossPosition) {
          match->logger.writeToFile("experimental.txt", "DEBUG - startedPassInCrossPosition = true\tside = " + int_to_str(player->GetTeam()->GetSide()) + "\tx = " + real_to_str(x) + "\ty = " + real_to_str(y));
        } else {
          match->logger.writeToFile("experimental.txt", "DEBUG - startedPassInCrossPosition = false\tside = " + int_to_str(player->GetTeam()->GetSide()) + "\tx = " + real_to_str(x) + "\ty = " + real_to_str(y));          
        }
            

        if (currentAnim->functionType == e_FunctionType_ShortPass) {          
          match->logger.writeToFile("experimental.txt", "frame: " + int_to_str(frame) + "\tstarted shortPass \tby player: " + int_to_str(player->GetID()));               
        } else if (currentAnim->functionType == e_FunctionType_LongPass) {
          match->logger.writeToFile("experimental.txt", "frame: " + int_to_str(frame) + "\tstarted longPass \tby player: " + int_to_str(player->GetID()));
        } else if (currentAnim->functionType == e_FunctionType_HighPass) {
          match->logger.writeToFile("experimental.txt", "frame: " + int_to_str(frame) + "\tstarted highPass \tby player: " + int_to_str(player->GetID()));
        }
        ...
        if (match->startedSavedShot) {
          // Se c'è un SavedShot in pending, lo posso annotare
          match->AnnotateSavedShot(match->startedShotFrame, match->savedShotEndFrame, match->startedShotShooter, match->savedShotPosition, match->savedShotGoalkeeper);             
          match->completedPass = false;      // Per eliminare l'evento PassThenGoal
        } else if (match->startedPass && match->startedPassPlayer->GetTeamID() == player->GetTeamID()) {     
          // Controllo se prima di iniziare questo tiro c'era un passaggio non segnato ancora      
          match->AnnotatePass(match->startedPassFrame, frame - 1, match->startedPassPlayer, player);
          match->AnnotateBallPossession(frame - 1, player);
          
          match->completedPass = true;
          match->startedPass = false;
          // TODO: controllare anche i cross e i filteringPass
        }     
        ...
        // Per valutare se il questo tiro può diventare un cross, se parte da una buona posizione        
        if (player->GetTeam()->GetSide() == -1) {
          // Se la squadra di player ha side == 1 allora la metà campo è quella destra
          if (x > pitchHalfW && (y < match->initCrossBottomLine || y > match->initCrossTopLine)) {
            // Se sono oltre la metà campo e sono sotto la linea della fascia oppure sopra la linea dell'altra fascia
            match->startedShotInCrossPosition = true;
            match->crossSide = true;
          }
        } else {
          // Altrimenti la metà campo è quella sinistra
        ...
        if (match->startedShotInCrossPosition) {
          match->logger.writeToFile("experimental.txt", "DEBUG - startedShotInCrossPosition = true\tside = " + int_to_str(player->GetTeam()->GetSide()) + "\tx = " + real_to_str(x) + "\ty = " + real_to_str(y));
        } else {
          match->logger.writeToFile("experimental.txt", "DEBUG - startedShotInCrossPosition = false\tside = " + int_to_str(player->GetTeam()->GetSide()) + "\tx = " + real_to_str(x) + "\ty = " + real_to_str(y));          
        }   
  
        match->startedShot = true;
        match->startedSavedShot = false;
        match->startedShotFrame = frame;
        match->startedShotShooter = player;
        match->savedShotShooter = player;
        match->savedShotPosition = player->GetPosition();
        ...
        match->logger.writeToFile("experimental.txt", "interfere \tby player: " + int_to_str(player->GetID()));
        ...
        match->AnnotateBallDeflection(player);

        // In caso di deflection a opera del portiere dopo che c'è in pending un evento shot, posso segnare startedSavedShot
        if (match->startedShot && player->GetFormationEntry().role == e_PlayerRole_GK) {
          // Controllo l'evento complesso SavedShot
          match->startedSavedShot = true;
          match->savedShotGoalkeeper = player;
          match->savedShotEndFrame = frame;          // Salvo il frame a cui è avvuta la deflection per usarla come frame finale per savedShot
        }
        ...
        int frame = match->GetActualTime_ms() / 10;
        Player* player = this->CastPlayer();
        match->logger.writeToFile("experimental.txt", "frame: " + int_to_str(frame) + "sliding \tby player: " + int_to_str(player->GetID()));
