...
void Player::Activate(boost::intrusive_ptr<Node> humanoidSourceNode, boost::intrusive_ptr<Node> fullbodySourceNode, std::map<Vector3, Vector3> &colorCoords, boost::intrusive_ptr < Resource<Surface> > kit, boost::shared_ptr<AnimCollection> animCollection) {
  ...
  /*
    // Lasciato qui solo per eventuale riferimento alle prove fatte
    Gui2Caption* nameCaption1, *nameCaption2, *nameCaption3, *nameCaption4, *nameCaption5;

    nameCaption1 = new Gui2Caption(GetMenuTask()->GetWindowManager(), "game_player_name_" + int_to_str(id), 0, 0, 1, 2.0, "5 5");
    nameCaption1->SetTransparency(0.3f);
    GetMenuTask()->GetWindowManager()->GetRoot()->AddView(nameCaption1);
    nameCaption1->SetPosition(0, 0);
    nameCaption1->Show();

    std::vector<boost::intrusive_ptr<blunted::Image2D>> *target = new std::vector<boost::intrusive_ptr<blunted::Image2D>> ();
    nameCaption1->GetImages(*target);

    for (unsigned int i = 0; i < target->size(); i++) {
      boost::intrusive_ptr<blunted::Image2D> &image = target->at(i);
      if (image != boost::intrusive_ptr<blunted::Image2D>()) {
        float x_percent, y_percent;
        int x, y, w, h;
        GetMenuTask()->GetWindowManager()->GetCoordinates(5, 5, 0, 0, x, y, w, h);
...
void Player::UpdatePossessionStats(bool onInterval) {
  ...
  if ((CastHumanoid()->GetCurrentFunctionType() == e_FunctionType_ShortPass ||
    ...
    //forse qui si potrebbe loggare l'evento così, usando CastHumanoid.....?
...
void Player::Process() {
  ...
  if (isActive) {
    ...
    //int x, y;
    //GetDebugOverlayCoord(match, GetGeomPosition(), x, y);
    //nameCaption->SetCaption(int_to_str(x) + " " + int_to_str(y));
...
void Player::FetchPutBuffers(unsigned long putTime_ms) {
  ...
  // Per abilitare i nomi sopra ogni giocatore, non solo quello attivo
  fetchedbuf_nameCaptionShowCondition = true;
...
//      GetMenuTask()->GetWindowManager()->GetCoordinates(p2.coords[0], p2.coords[1], 1, 1, x2, y2, dud1, dud2);
  ...
  int a1 = x + 150;
  Line line;
  line.SetVertex(0, Vector3(x, y, 0));
  line.SetVertex(1, Vector3(a1, y, 0));
  GetDebugOverlay()->DrawLine(line, Vector3(255, 255, 255), 128);

  Vector3 color(255, 100, 100);
  GetDebugOverlay()->DrawRectangle(x - 20, y - 75, 40, 75, color, 255);
  */
...
void Player::Log2DPositionTimestamp() {
  int x, y;
  GetDebugOverlayCoord(match, GetGeomPosition(), x, y);
  
  int base = 0;
  if (GetTeamID() == 1) base = 128;

  std::string frame = int_to_str(match->GetActualTime_ms() / 10);
  
  string lineStr = std::to_string(match->getCurrentRelativeTimestampMillis()) + " " + frame + " " + int_to_str(GetID() + base) + " " + int_to_str(x) + " " + int_to_str(y);
  match->timestamp2dPositionLogFile << lineStr << std::endl;
}
